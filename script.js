...
function updateLoadingProgress() {
  ...
  if (loadedAssets >= totalAssets) {
    // Tout est chargé, on peut initialiser le jeu
    setTimeout(() => {
...
// Jouer un son avec contrôle du volume
function playSound(key, volume = 1.0) {
  if (!config.soundEnabled || !resourceCache.sounds[key]) return;

  // Cloner le son pour pouvoir jouer plusieurs instances en même temps
...
function initMap() {
  console.log("Initialisation de la carte aléatoire...");
  ...
  // Générer la carte de manière aléatoire
  ...
  // Ajouter des villageois de départ
  ...
  console.log("Carte créée aléatoirement:", config.mapWidth, "x", config.mapHeight);
}
...
// Ajouter des ressources à la carte
...
function spawnUnit(x, y, type) {
  // Vérifier les limites de population
...
function spawnVillager() {
  if (gameState.resources.food >= 5) {
    // Trouver un bâtiment de type mairie pour faire apparaître des villageois
    ...
    if (unit) {
      ...
      updateUI();
      updateStatus("Villageois créé");
    }
  } else {
    updateStatus("Pas assez de nourriture pour créer un villageois");
    playSound('errorSound', 0.5);
  }
}
  ...
  if (gameState.resources.food >= 10 && gameState.resources.wood >= 5) {
    ...
    if (barracks) {
      ...
      if (unit) {
        ...
        updateUI();
        updateStatus("Soldat créé");
      }
    } else {
      updateStatus("Vous avez besoin d'une caserne pour créer des soldats");
      playSound('errorSound', 0.5);
    }
  } else {
    updateStatus("Ressources insuffisantes pour créer un soldat");
    playSound('errorSound', 0.5);
  }
}
...
function spawnArcher() {
  if (gameState.resources.food >= 10 && gameState.resources.wood >= 10) {
    // Vérifier si un stand de tir existe
    ...
    if (archery) {
      ...
      if (unit) {
        ...
        updateUI();
        updateStatus("Archer créé");
      }
    } else {
      updateStatus("Vous avez besoin d'un stand de tir pour créer des archers");
      playSound('errorSound', 0.5);
    }
  } else {
    updateStatus("Ressources insuffisantes pour créer un archer");
    playSound('errorSound', 0.5);
  }
}
  ...
  if (gameState.resources.food >= 20 && gameState.resources.stone >= 15) {
    ...
    if (barracks) {
      ...
      if (unit) {
        ...
        updateUI();
        updateStatus("Chevalier créé");
      }
    } else {
      updateStatus("Vous avez besoin d'une caserne pour créer des chevaliers");
      playSound('errorSound', 0.5);
    }
  } else {
    updateStatus("Ressources insuffisantes pour créer un chevalier");
    playSound('errorSound', 0.5);
  }
}

function spawnEnemy() {
  // Faire apparaître des ennemis sur les bords de la carte
...
function buildStructure(type) {
  // Définir les coûts et dimensions des bâtiments
  let buildingInfo = {
    width: 1,
    height: 1,
    woodCost: 0,
  ...
  updateStatus(`Sélectionnez un emplacement pour construire: ${getBuildingName(type)}`);
...
function getBuildingName(type) {
  switch(type) {
    ...
    case 'towncenter': return 'Hôtel de ville';
    default: return type;
  }
}
...
function placeBuildingAt(x, y, type, width, height, populationProvided = 0) {
  ...
  // Si l'emplacement est valide, ajouter le bâtiment
  const building = {
    x, y,
    width, height,
    type,
  ...
  if (gameState.buildMode) {
    ...
    // Ajouter à la population maximale si c'est une maison
...
function updateUI() {
  ...
  // Mettre à jour les boutons en fonction des ressources disponibles
...
function gameLoop(timestamp) {
  ...
  // Mettre à jour les unités, bâtiments, etc.
  updateGameState();
...
function updateGameState() {
  // Interpoler la caméra pour un mouvement fluide
  ...
  // Mettre à jour les ressources automatiques
...
function updateUnits() {
  ...
  for (const villager of gameState.units.villagers) {
    ...
    if (villager.x !== villager.targetX || villager.y !== villager.targetY) {
      ...
      // Déterminer la direction du mouvement pour l'animation
    ...
    if (villager.task === 'gather' && villager.taskTarget) {
      ...
      // Si à proximité de la ressource
    ...
    if (villager.task === 'build' && villager.taskTarget) {
      ...
      // Si à proximité du bâtiment
      if (dist <= 1.5) {
        // Orienter le villageois vers le bâtiment
        ...
        if (villager.taskTarget.constructionProgress >= 100) {
          ...
          // Ajouter un effet de construction terminée
          addEffect('constructionComplete', bx, by, 1000);
        }
      }
    }

    // Auto-défense contre les ennemis proches
    ...
    // Mettre à jour le temps de recharge d'attaque
    if (villager.attackCooldown > 0) {
      villager.attackCooldown -= gameState.deltaTime;
    }
  }

  // Mise à jour des soldats, archers et chevaliers de façon similaire
...
function updateCombatUnits(units, unitType) {
  for (const unit of units) {
    ...
    // Attaque automatique des ennemis à proximité
    if (gameState.enemies.length > 0) {
      ...
      if (closestEnemy) {
        // Orienter l'unité vers l'ennemi
        ...
        // Attaquer si le temps de recharge est écoulé
        if (unit.attackCooldown <= 0) {
          // Déterminer les dégâts selon le type d'unité
          let damage = 0;
          switch (unitType) {
...
function updateBuildings() {
  for (const building of gameState.buildings) {
    // Si le bâtiment est en construction, pas de production
    if (building.isConstructing) continue;

    // Production automatique selon le type de bâtiment
...
function updateEnemies() {
  for (let i = gameState.enemies.length - 1; i >= 0; i--) {
    ...
    // Vérifier les bâtiments d'abord
    for (const building of gameState.buildings) {
      if (building.isConstructing) continue; // Ignorer les bâtiments en construction
    ...
    // Ensuite vérifier toutes les unités
    const allUnits = [
    ...
    if (closestTarget) {
        ...
        if (enemy.attackCooldown <= 0) {
          if (closestTarget.type === 'building') {
            ...
            if (closestTarget.obj.hp <= 0) {
              // Si c'est une maison, réduire la population maximale
      ...
      // Pas de cible, se déplacer aléatoirement
      if (Math.random() < 0.01) {
...
function addResourceIndicator(x, y, type, amount) {
  ...
  // Définir le contenu selon le type de ressource
  let icon;
  switch (type) {
    case 'food': icon = '🌾'; break;
    case 'wood': icon = '🪵'; break;
    case 'stone': icon = '⛏️'; break;
    default: icon = '+';
  }
...
function collectAutomaticResources() {
  // Cette fonction gère les ressources générées automatiquement par certains bâtiments
  // La logique est déjà implémentée dans updateBuildings
}

function updateAnimations() {
  // Cette fonction met à jour les frames d'animation pour les unités et effets
  // La logique est déjà implémentée dans les fonctions respectives
}
...
function drawMap() {
  ...
  for (let y = startY; y < endY; y++) {
    for (let x = startX; x < endX; x++) {
      ...
      // Dessiner les ressources à cette position
...
function drawBuildings() {
  ...
  for (const building of gameState.buildings) {
    ...
    if (building.isConstructing) {
      ...
      let symbol = '';
      switch(building.type) {
        case 'house': symbol = '🏠'; break;
        case 'farm': symbol = '🌾'; break;
        case 'lumbermill': symbol = '🪓'; break;
        case 'barracks': symbol = '⚔️'; break;
      }
...
function drawUnitType(units, type) {
  ...
  for (const unit of units) {
    ...
    // Dessiner selon le type d'unité
    if (type === 'villager') {
      // Villageois (cercle)
...
function drawEnemies() {
  ...
  for (const enemy of gameState.enemies) {
    ...
    // Vérifier si l'ennemi est dans la zone visible
    if (screenX + tileSize < 0 || screenX > canvas.width ||
        screenY + tileSize < 0 || screenY > canvas.height) {
      continue;
    }

    // Vérifier si l'ennemi est visible (brouillard de guerre)
    ...
    // Si l'ennemi est visible mais en brouillard de guerre exploré
...
function drawEffects() {
  ...
  for (const effect of gameState.effectsQueue) {
    ...
    // Vérifier si l'effet est dans la zone visible
    if (screenX + tileSize < 0 || screenX > canvas.width ||
        screenY + tileSize < 0 || screenY > canvas.height) {
      continue;
    }
...
function drawUI() {
  ...
  // Dessiner le mode construction (aperçu du bâtiment)
  if (gameState.buildMode) {
    const tileSize = config.tileSize;
    ...
    // Dessiner un rectangle coloré pour indiquer la validité
...
function drawMinimap() {
  ...
  for (let y = 0; y < config.mapHeight; y++) {
    for (let x = 0; x < config.mapWidth; x++) {
      ...
      // Appliquer l'effet de brouillard de guerre en mode exploré
...
// Événements de souris et entrées utilisateur
...
function handleMouseDown(e) {
  ...
  // Clic droit pour déplacer les unités ou collecter des ressources
  if (e.button === 2) {
    ...
    // Si des villageois sont sélectionnés et qu'il y a une ressource
    if (resource && gameState.selected.some(unit => unit.type === 'villager')) {
      ...
      updateStatus(`${gameState.selected.filter(u => u.type === 'villager').length} villageois assignés à la récolte de ${resource.type}`);
    }
    // Si des villageois sont sélectionnés et qu'il y a un bâtiment en construction
      ...
      updateStatus(`${gameState.selected.filter(u => u.type === 'villager').length} villageois assignés à la construction`);
      playSound('villagerAcknowledge', 0.4);
    }
    // Sinon, déplacer les unités sélectionnées
    else if (gameState.selected.length > 0) {
      // Vérifier si le clic est sur un ennemi pour attaquer
      ...
      for (const enemy of gameState.enemies) {
        ...
        if (tileX === enemyX && tileY === enemyY) {
          // Ordonner aux unités de combat d'attaquer
          for (const unit of gameState.selected) {
            // Les villageois peuvent aussi attaquer mais avec moins d'efficacité
            unit.task = 'attack';
            unit.taskTarget = enemy;
            unit.targetX = enemy.x;
            unit.targetY = enemy.y;

            // Son d'acknowledgement selon le type d'unité
            if (unit.type === 'villager') {
              playSound('villagerAcknowledge', 0.4);
            } else {
          ...
          enemyClicked = true;
          updateStatus(`${gameState.selected.length} unités attaquent l'ennemi`);
          break;
        }
      }

      // Si pas d'ennemi, juste déplacer
      if (!enemyClicked) {
        // Répartir les unités autour du point cible pour éviter qu'elles se superposent
        const numSelected = gameState.selected.length;
        const radius = Math.sqrt(numSelected) * 0.5;
        let index = 0;

        for (const unit of gameState.selected) {
          // Calculer un décalage pour chaque unité
          ...
          // Réinitialiser la tâche si l'unité en avait une
          unit.task = null;
        ...
        updateStatus(`${numSelected} unités en déplacement`);
      }
    }
  ...
  if (gameState.buildMode) {
    ...
    if (placeBuildingAt(worldX, worldY,
      ...
      updateStatus(`Construction de ${getBuildingName(gameState.buildMode.type)} commencée`);

      // Assigner des villageois sélectionnés à la construction
      for (const unit of gameState.selected) {
        if (unit.type === 'villager') {
          unit.task = 'build';
      ...
      updateStatus("Impossible de construire à cet endroit");
      playSound('errorSound', 0.5);
    }
...
function handleMouseMove(e) {
  ...
  // Mettre à jour la boîte de sélection si on est en train de sélectionner
  if (gameState.selectionBox) {
    gameState.selectionBox.endX = x;
    gameState.selectionBox.endY = y;
  }

  // Déplacer la caméra si la souris est près des bords
  const edgeSize = 30;
...
function handleMouseUp(e) {
  // Si on était en train de faire une sélection
  if (gameState.selectionBox) {
    ...
    // Si la boîte est très petite, considérer comme un clic simple
    if (Math.abs(endX - startX) < 5 && Math.abs(endY - startY) < 5) {
      ...
      // Vérifier si une unité est cliquée
      const allUnits = [
        ...gameState.units.villagers,
        ...gameState.units.soldiers,
        ...gameState.units.archers,
      ...
      for (const unit of allUnits) {
        ...
        if (unitX === worldX && unitY === worldY) {
          ...
          break; // Sélectionner seulement une unité en cas de clic simple
        }
      }
    } else {
      // Sélectionner les unités dans la boîte
      const allUnits = [
        ...gameState.units.villagers,
        ...gameState.units.soldiers,
        ...gameState.units.archers,
      ...
      for (const unit of allUnits) {
        ...
        // Vérifier si l'unité est dans la boîte de sélection
